package com.amu.chen.keeping.rabbitmq;
...
public class MQConfig {
    ...
    // 模式是根据 Config 中设置的 RoutineKey 还有发送消息时候的 topic 来判断是否会传输。
    @Bean
    ...
    // Fanout模式：广播
    // 模式类似于广播，不用设置路由，只要发送消息设置了对应的 Exchange 就可以对该 Exchange 中的接收者进行广播
    @Bean
    ...
    // 模式比较不同于其他三种模式。Headers 是一个键值对，可以定义成 Hashtable。发送者在发送的时候定义一些键值对，接收者也可以再绑定时候传入一些键值对，两者匹配的话，则对应的队列就可以收到消息。匹配有两种方式all和any
    @Bean
